...
- Leave flow: `sendLeaveRequest()` broadcasts ControlLeave with `keyAuth` HMAC and the sender MAC. Receivers verify the sender/payload MAC match, emit `onJoinEvent(mac,false,false)`, and drop the peer immediately (no heartbeat delay). The sender also purges its local peers after enqueueing the leave.
