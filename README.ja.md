...
- Serial 層を追加可能: `EspNowSerial` と `EspNowSerialPort` により、`EspNowBus` の上に `Stream` / `Print` 互換のシリアル抽象を載せられる。
- IP 層を追加可能: `EspNowIP` と `EspNowIPGateway` により、`EspNowBus` の上に `esp_netif` ベースの `IPv4` / `routing + NAT` 構成を載せられる。
...
このリポジトリには、`EspNowBus` の上に載る軽量なシリアル層 `EspNowSerial` も含まれています。

- Hub クラス: `EspNowSerial`
- Port クラス: `EspNowSerialPort`
- API スタイル: Arduino `Stream` / `Print`

Serial 系のサンプルは [`examples/Serial`](examples/Serial) にまとめています。
...
これらのサンプルでは、直接接続、controller/device bridge、複数 session 監視、外部シリアル CLI ライブラリの ESP-NOW bridge まで確認できます。

## IP over EspNow

このリポジトリには、`EspNowBus` の上に載る軽量な IP 層 `EspNowIP` も含まれています。

- device クラス: `EspNowIP`
- gateway クラス: `EspNowIPGateway`
- 想定モデル: `device -> gateway -> uplink`
- 現在の対象: `IPv4`, `routing + NAT`
- 実装方式: device 側は仮想 `esp_netif`、gateway 側は bus 側 `esp_netif` と uplink 側 `esp_netif` を接続する `L3 gateway`

### IP の考え方
- `EspNowIP` は `EspNowBus` の上に `IP session` を張り、device 側では通常のネットワークインターフェイスのように `lwIP` から利用できます。
- gateway は `L2 bridge` ではなく `L3 gateway` として動作し、bus 側を内側、uplink 側を外側として `routing + NAT` を行います。
- device が到達できる先は、基本的に gateway 自身と gateway の先にある uplink 側ネットワークです。device 間の IP 転送は行いません。
- 複数 gateway を置くことで冗長化はできますが、接続先は固定ではなく、切断後に再接続した時点で成立した gateway を使います。無停止切替ではありません。
- `EspNowBus` の再送や app-ack の上に `TCP` が載るため、低遅延や高スループットを求める用途には向きません。`HTTP` や `MQTT` のような低速で再接続前提の用途を想定しています。
- サンプルや既定設定は PoC と構成例を主目的としており、スループット最適化や長時間運転の追い込みはまだ限定的です。

IP 系のサンプルは [`examples/IP`](examples/IP) にまとめています。
...
これらのサンプルでは、device 側の lease 取得、gateway 側の Wi-Fi STA / Ethernet / PPP uplink、`gateway ping` / DNS / NTP / HTTP による疎通確認まで扱えます。

関連ドキュメント:
- IP サンプル README: [`examples/IP/README.ja.md`](examples/IP/README.ja.md)
